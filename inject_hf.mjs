...
async function injectHF() {
    ...
    const ingestorPayload = {
        "name": "[REAL-TIME] WhatsApp Ingestor 📥💎",
        "nodes": [
            {
                "parameters": {
                    "httpMethod": "POST",
    ...
    console.log('--- Buscando y Borrando Workflows Anteriores ...');
    ...
    console.log(`
🎉 INYECCIÓN COMPLETADA. LOS WORKFLOWS DE HUGGING FACE ESTÁN ACTIVOS Y BLINDADOS.

🔗 EL LINK SECRETO DE TU WEBHOOK OFICIAL (REEMPLAZA ESTO EN EVOLUTION API):
https://${HF_HOST}/webhook/whatsapp-webhook
`);
}
