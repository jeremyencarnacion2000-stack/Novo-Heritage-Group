...
// V40.9: ESTABILIZACIÓN DEFINITIVA (MODELOS + SANITIZACIÓN)
const nodes = [
  {
    id: 'trigger',
    ...
    parameters: {
      jsCode: `
        return $input.all().map(item => {
          ...
          // SANITIZACIÓN ROBUSTA: ESCAPADO MANUAL DE CARACTERES JSON
          const escape = (str) => {
            if (!str) return '';
            return str
              .replace(/\\\\/g, '\\\\\\\\')
  ...
  },
  {
    ...
    parameters: {
      ...
      headerParameters: { parameters: [{ name: 'Authorization', value: 'Bearer 09b89a76-2d1c-4a47-863e-f4a5389e6ad8' }] }
    }
  },
  {
    id: 'text-ai',
...
function deploy() {
  const data = JSON.stringify({ name: 'PROD: Real Estate Pipeline (V40.9 Stabilized)', nodes, connections, settings: { executionOrder: 'v1' } });
  const req = https.request({
    ...
    res.on('end', () => {
      console.log('=== V40.9 DEPLOYED (MODELS FIXED) ===');
