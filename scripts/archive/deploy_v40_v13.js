...
// V40.13: BYPASS HTTP NODES -> USE CODE NODES FOR AI CALLS
const nodes = [
  {
    id: 'trigger',
        ...
        try {
          const response = await axios.post('https://api.sambanova.ai/v1/chat/completions', {
            ...
            messages: [
              {
                role: "user",
                content: "Extrae JSON inmobiliario: {\\"proyecto\\":\\"...\\",\\"zona\\":\\"...\\",\\"precio\\":\\"...\\",\\"habitaciones\\":\\"...\\",\\"amenidades\\":\\"...\\"}. Texto: " + item.text_content
              }
            ]
          }, {
            headers: {
              'Authorization': 'Bearer ' + SAMBANOVA_API,
              'Content-Type': 'application/json'
            }
          });
          return { json: { ...item, ...response.data } };
        } catch (error) {
          return { json: { error: error.message, status: error.response?.status, detail: error.response?.data } };
        }
      `
    }
  },
  ...
  },
  {
    ...
    parameters: {
      jsCode: `
        const item = $input.first().json;
        if (item.error) return { json: { skip: true, reason: item.error } };
...
function deploy() {
  const data = JSON.stringify({ name: 'PROD: Real Estate Pipeline (V40.13 Master Bypass)', nodes, connections, settings: { executionOrder: 'v1' } });
  const req = https.request({
    ...
    res.on('end', () => {
      console.log('=== V40.13 DEPLOYED (BYPASS) ===');
