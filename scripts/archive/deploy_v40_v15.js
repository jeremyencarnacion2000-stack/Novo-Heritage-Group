...
// V40.15: MASTER RESTORATION (HTTP NODES + HARDCODED BEARER)
const nodes = [
  {
    id: 'trigger',
    ...
    parameters: {
      jsCode: `
        return $input.all().map(item => {
          ...
          return { json: { ...item.json, text_content: escape(text) } };
        });
      `
    }
  },
  ...
  },
  {
    ...
    parameters: {
      ...
      sendBody: true,
      specifyBody: 'json',
      jsonBody: '={ "model": "Meta-Llama-3.3-70B-Instruct", "messages": [ { "role": "user", "content": "Extrae JSON inmobiliario: {\\"proyecto\\":\\"...\\",\\"zona\\":\\"...\\",\\"precio\\":\\"...\\",\\"habitaciones\\":\\"...\\",\\"amenidades\\":\\"...\\"}. Texto: " + $json.text_content } ] }',
  ...
  },
  {
    ...
    parameters: {
      jsCode: `
        ...
        if (!item.choices || !item.choices[0]) return { json: { skip: true } };
...
function deploy() {
  const data = JSON.stringify({ name: 'PROD: Real Estate Pipeline (V40.15 Master Restoration)', nodes, connections, settings: { executionOrder: 'v1' } });
  const req = https.request({
    ...
    res.on('end', () => {
      console.log('=== V40.15 DEPLOYED (MASTER) ===');
