...
async function autofixDeploy() {
    console.log('--- DEPLOYING V14 (SYNTAX FIX) ---');

    const ACTUAL_PG_ID = "ym3KfQZLjCvWmUjS";
    const ACTUAL_GROQ_ID = "Bz3xfXM6claHNoqr";

    // Hemos quitado el "=" inicial en Fetch_50 y Save_Project
    const payload = {
        name: "[AUTOFIX] Godmode Backfill V14 🏡🛡️💎",
        nodes: [
            ...
            { "parameters": { "operation": "executeQuery", "query": "SELECT * FROM evolution.\"Message\" ORDER BY \"messageTimestamp\" DESC LIMIT 50 OFFSET {{$json.offset}};" }, "name": "Fetch_50", "type": "n8n-nodes-base.postgres", "typeVersion": 1, "position": [400, 300], "credentials": { "postgres": { "id": ACTUAL_PG_ID } } },
    ...
    console.log('Borrando V13 y V12...');
    await request('/workflows/oixKorUBMFdGQsSS', 'DELETE'); // V13
    await request('/workflows/U1vvMdYeDHvFUTvi', 'DELETE'); // V12 (just in case)

    console.log('Inyectando V14 con Sintaxis Corregida...');
    const result = await request('/workflows', 'POST', payload);
    if (!result.id) {
        console.error('Error al crear V14:', result);
        return;
    }
    console.log('✅ V14 Creado:', result.id);

    // Activar
    await request(`/workflows/${result.id}/activate`, 'POST');
    console.log('✅ V14 ACTIVADO.');
}
