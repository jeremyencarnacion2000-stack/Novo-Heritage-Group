...
async function deepRepair() {
    try {
        ...
        console.log('Aplicando variables de entorno de compatibilidad profunda...');
        await request('PUT', `/services/${sid}/env-vars`, vars);

        console.log('Disparando despliegue de emergencia...');
        await request('POST', `/services/${sid}/deploys`);

        console.log('SISTEMA EN REANIMACIÓN.');
    } catch (err) {
