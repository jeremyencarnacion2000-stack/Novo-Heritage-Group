...
async function fixV13() {
    console.log('--- DEPLOYING V13 ABSOLUTE ---');

    const ACTUAL_PG_ID = "ym3KfQZLjCvWmUjS";
    const ACTUAL_GROQ_ID = "Bz3xfXM6claHNoqr";

    const payload = {
        name: "[ABSOLUTE] Godmode Backfill V13 🏡🛡️💎",
        nodes: [
    ...
    console.log('Borrando V12 y anteriores...');
    await request('/workflows/U1vvMdYeDHvFUTvi', 'DELETE');

    console.log('Creando V13...');
    const result = await request('/workflows', 'POST', payload);
    if (!result.id) {
        console.error('Error al crear V13:', result);
        return;
    }
    console.log('✅ V13 Creado con ID:', result.id);

    // Activar
    await request(`/workflows/${result.id}/activate`, 'POST');
    console.log('✅ V13 Activado.');

    // Ejecutar
    console.log('Disparando primera ejecución...');
    const runRes = await request(`/workflows/${result.id}/run`, 'POST');
    console.log('🚀 Trigger lanzado.');
