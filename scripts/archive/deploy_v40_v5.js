...
// V40.5: CORRECCIÓN DE FORMATO JSON PARA IA (ULTRA ESTABLE)
const nodes = [
  {
    id: 'trigger',
    ...
    parameters: {
      ...
      jsonBody: `={
        ...
        "messages": [
          {
            "role": "user",
            "content": "Extrae JSON inmobiliario: {\\"proyecto\\":\\"...\\",\\"zona\\":\\"...\\",\\"precio\\":\\"...\\",\\"habitaciones\\":\\"...\\",\\"amenidades\\":\\"...\\"}. Texto: {{ $node[\\"Item Splitter\\"].json[\\"text_content\\"] }}"
          }
        ]
      }`,
      headerParameters: { parameters: [{ name: 'Authorization', value: `Bearer ${SAMBANOVA_API}` }] }
  ...
  },
  {
    ...
    parameters: {
      jsCode: `
        ...
        if (!ai.precio || String(ai.precio) === '...') return { json: { skip: true } };
...
function deploy() {
  const data = JSON.stringify({ name: 'PROD: Real Estate Pipeline (V40.5 Fix)', nodes, connections, settings: { executionOrder: 'v1' } });
  const req = https.request({
    ...
    res.on('end', () => {
      console.log('=== V40.5 DEPLOYED (AI JSON FIX) ===');
