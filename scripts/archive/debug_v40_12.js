...
async function debugV4012() {
  const exID = '2669'; // Última ejecución V40.12
        ...
        for (const node in runData) {
            runData[node].forEach(run => {
                if (run.error) {
                    ...
                    if (run.error.response?.body) {
                        console.log('Response Body:', JSON.stringify(run.error.response.body));
                    }
                }
            });
        }
