...
async function debugV406() {
  ...
  if (res.s === 200) {
    ...
    if (data.data && data.data.resultData && data.data.resultData.error) {
        console.log('Error Message:', data.data.resultData.error.message);
    }
        ...
        Object.keys(runData).forEach(node => {
            runData[node].forEach(run => {
                if (run.error) {
                    ...
                    console.log('Expression Value (if AI):', JSON.stringify(run.error.description || '', null, 2));
                }
            });
        });
    }
