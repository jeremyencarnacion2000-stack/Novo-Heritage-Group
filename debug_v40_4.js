...
async function debugV404() {
  ...
  if (res.s === 200) {
    ...
    // Print the global error if exists
    if (data.data && data.data.resultData && data.data.resultData.error) {
        console.log('Global Error:', data.data.resultData.error.message);
    }
    // Check specific node errors
    const runData = data.data && data.data.resultData && data.data.resultData.runData;
    if (runData) {
        for (const node in runData) {
            runData[node].forEach(run => {
                if (run.error) console.log(`Node [${node}] failed with: ${run.error.message}`);
            });
        }
    }
  } else {
